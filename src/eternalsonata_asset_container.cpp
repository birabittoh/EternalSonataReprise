...
bool ParseBtx(const std::vector<uint8_t>& d, size_t base, BtxBlob* out) {
  ...
  for (uint32_t i = 0; i < nlangs; ++i) {
    ...
    for (uint32_t k = 0; k < count; ++k) {
      ...
      if (stop >= end)
        return false;
      lang.entries.emplace(
          sid, std::string(reinterpret_cast<const char*>(d.data() + start), stop - start));
    }
    if (i == 0)
      out->f12 = lang.f12;
    if (i + 1 == nlangs)
...
bool ReplaceContainerRange(std::vector<uint8_t>& data, size_t offset, size_t old_size,
  ...
  if (offset > data.size() || old_size > data.size() - offset)
    return false;
  const int64_t delta = int64_t(replacement.size()) - int64_t(old_size);
  std::vector<size_t> ancestors;
  static constexpr const char* kContainerMagics[] = {"NOBJ", "NMDL"};
  for (size_t at = 0; at + 8 <= offset; ++at) {
    bool known = false;
    for (const char* magic : kContainerMagics)
      known |= std::memcmp(data.data() + at, magic, 4) == 0;
    if (!known)
      continue;
    uint32_t size = 0;
    if (ReadBE32At(data, at + 4, &size) && size >= 8 && size <= data.size() - at && at < offset &&
        at + size >= offset + old_size)
      ancestors.push_back(at);
  }
  std::vector<uint8_t> out;
  out.reserve(size_t(int64_t(data.size()) + delta));
  out.insert(out.end(), data.begin(), data.begin() + ptrdiff_t(offset));
  out.insert(out.end(), replacement.begin(), replacement.end());
  out.insert(out.end(), data.begin() + ptrdiff_t(offset + old_size), data.end());
  data.swap(out);
  if (delta) {
    for (size_t at : ancestors) {
      uint32_t size = 0;
      if (ReadBE32At(data, at + 4, &size))
        WriteBE32(data.data() + at + 4, uint32_t(int64_t(size) + delta));
    }
    FixRelocations(data, offset, uint32_t(old_size), delta);
    ShiftHeader(data, delta);
  }
  return true;
}
