...
//
// The last piece of the ten-to-twelve conversion, and the easy one. Two
// functions gate on the character id before doing anything, and both reject
// anything outside 1..10 outright:
//
...
//
// Both are `cmpwi`/`cmplwi` against a literal, which a mid-ASM hook cannot
// rewrite. So both hooks fire *before* the compare and take the accept path
// themselves via jump_address_on_true when the id is one of the two new ones,
// leaving the original instruction to handle the first ten exactly as before.
// Jumping past the compare leaves cr6 holding whatever it held; in both cases
// the next reader of cr6 on the accept path is preceded by its own compare (or
// by a call, which makes cr6 volatile anyway), so nothing observes the skip.
//
...
// for the same reason the rest of the conversion is not. Addresses and options:
//
...
//
// Both fire before the instruction at the listed address.
